...
function Control(botsan) {
              ...
              if (!vrified) {
                ...
                if (!missing) {
                  ...
                  if (!formData.torrenturl)
                    anime = new Anime(formData.title, formData.prefix, formData.regex, formData.nyaasearch, formData.nyaauser, formData.uploadsID, formData.quality);
                  else
                    anime = new Anime(formData.title, formData.prefix, formData.regex, null, null, formData.uploadsID, formData.quality, null, formData.torrenturl);
  ...
  io.on('connection', function (socket) {
    ...
    botsan.myEmitter.on('writeData', () => {
      socket.emit('news', {
        application_status: botsan.application_status,
        episode_status: botsan.episode_status
      });
    });
...
function getAnimeInHtml(animelist) {
  var transform = {
    "<>": "ul", "html": [
      ...
      {
        "<>": "li",
        "html": "Finished episodes: ${finished_episodes}",
        "children": function () {
