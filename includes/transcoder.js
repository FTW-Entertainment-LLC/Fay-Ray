...
function Transcoder(botsan) {
  ...
  this.options = {
    ...
    audioBitrate: 128,
    preset: 'medium',
    passlog: `${botsan.config.paths.temp}/`,
    width: 854,
    height: 480,
    transcoder: this
  };
...
//TODO: (IMPORTANT) Check if ffmpeg hasn't returned a progress in one minute,
// it's most likely stuck. Restart
//TODO: (Not important) Support for 4/3 aspect ratio.

Transcoder.prototype.processFile = function processFile(file_in,
                                                        options,
                                                        callback) {
  //TODO: Check video stream, if it can be rewrapped instead of transcoded.
  //TODO: ^Same for audio (More important)
  //TODO: Check for several audio tracks, choose the one most suited. i.e
  // japanese over any other language.
  //TODO: Check for several subtitles tracks, choose the one most suited.
  // i.e english over any other language.
  //TODO: Allow choosing specific language tracks. This will have to be
  // somehow controlled in the Web GUI (Sometimes they're mixed, where english
  // is japanese, and japanese is english)
  //NOTE: Everything above is only suited for finished series. Most airing
  // series only has one audio and subtitle track.
  ...
  this.ffmpeg.ffprobe(file_in, function (err, metadata) {
    ...
    //TODO: FFMPEG can calculate width automatically, so I can remove the
    // width option. However this calculation requires the width. Calculate
    // the width by the aspect ratio from the ffprobe data
    ...
    //fluent-ffmpeg won't return a bitrate, so I couldn't use that value
    // with the file I was testing with.
    ...
    if (videos.length > 0) {
      ...
      //TODO: Choose from selected audio stream, when that's supported
      // (See further above)
...
Transcoder.prototype.run = function run(source, episode, callback) {
  ...
  for (var i = 0; i < resolutions.length; i++) {
    var task = {
      ...
      file_in: source,
      file_out: `${
        this.botsan.config.paths.temp
        }/${
        this.botsan.createFilename(
...
Transcoder.prototype.transcode = function transcode(file_in,
                                                    file_out,
                                                    resolution,
                                                    options,
                                                    callback) {
  ...
  this.processFile(file_in, options, function onProcessDone() {
    var command = new ffmpeg(file_in)
      ...
      .addOptions([
        "-passlogfile",
        options.passlog +
        options.transcoder.botsan.path.parse(file_out).name
      ])
      ...
      .on('error', function onError(err) {
        transcoder.FFmpegOnError(err)
      })
      .on('progress', function onProgress(p) {
        transcoder.FFmpegOnProgress(p)
      })
      .on('start', function onStart(cmd) {
        transcoder.FFmpegOnStart(cmd)
      })
      .on('end', function onFfmpegEnd() {
        this.lastprogress_frm = null;
        ...
        options.transcoder.botsan.fs.rename(
          file_out,
          target,
          function (err) {
            if (err)
...
Transcoder.prototype.transcode_task = function transcode_task(task, callback) {
  task.transcoder.transcode(
    task.file_in,
    task.file_out,
    task.resolution,
    task.transcoder.options,
    callback
  );
}
...
Transcoder.prototype.FFmpegOnStart = function FFmpegOnStart(commandLine) {
  ...
  //pass_interval =
  // setInterval(restartFFmpeg(mylastprogress_frm, this.lastprogress_frm),
  // 10000);
